...
slack_penalty <- max(dissatisfaction_values) + 100
...
# assignment_table_post_processing <- assignment_table
# doctor_names <- rownames(preference_matrix)
# 
# ## SWAP FOR SPECIALTY TERM HETI ADHERANCE ##
# 
# find_suitable_swap <- function(term_to_swap, doctor_id, preference_matrix, specialty_status_per_term) {
#   for (swap_candidate_doc in 1:nrow(preference_matrix)) {
#     if (swap_candidate_doc == doctor_id) next
#     
#     for (swap_candidate_term in assignment_table_post_processing[swap_candidate_doc, -1]) {
#       if (term_to_swap == swap_candidate_term) next
#       
#       term1_specialty <- specialty_status_per_term[term_to_swap]
#       term2_specialty <- specialty_status_per_term[swap_candidate_term]
# 
#       if (is.na(term1_specialty) || is.na(term2_specialty) || term1_specialty == term2_specialty) next
# 
#       original_preference_doc = preference_matrix[doctor_id, term_to_swap]
#       swap_preference_doc = preference_matrix[doctor_id, swap_candidate_term]
#       original_preference_swap_candidate = preference_matrix[swap_candidate_doc, swap_candidate_term]
#       swap_preference_swap_candidate = preference_matrix[swap_candidate_doc, term_to_swap]
# 
#         pref_diff_doc = swap_preference_doc - original_preference_doc
#   pref_diff_swap_candidate = swap_preference_swap_candidate - original_preference_swap_candidate
#   
#   # Make sure to use a positive differential regardless of which term is preferred.
#   combined_pref_diff = abs(pref_diff_doc) + abs(pref_diff_swap_candidate)
# 
#   # Check if the swap is within the acceptable preference difference for both doctors
#   if (combined_pref_diff <= 7 ) {
#     return(list(
#       swap_candidate_doc = swap_candidate_doc,
#       swap_candidate_term = swap_candidate_term,
#       combined_pref_diff = combined_pref_diff
#     ))
#   }
#     }
#   }
...
# }
# 
# # Initialize a dataframe to keep track of the swaps
# swap_df <- data.frame(DoctorA = character(),
#                       TermA = character(),
#                       DoctorB = character(),
#                       TermB = character(),
#                       PreferenceDifferential = numeric(),
#                       stringsAsFactors = FALSE)
# 
# # Use the function in the post-processing step
# violating_docs = which(!specialty_compliance)
# for (doc_idx in violating_docs) {
#   doc_name <- doctor_names[doc_idx]
#   doc_assignments <- unlist(assignment_table_post_processing[doc_idx, -1])
#   for (term in doc_assignments) {
#     specialty_counts <- table(specialty_status_per_term[doc_assignments])
#     term_specialty <- specialty_status_per_term[term]
# 
#     if (is.na(term_specialty) || specialty_counts[term_specialty] <= 2) next
# 
#     swap <- find_suitable_swap(term, doc_idx, preference_matrix, specialty_status_per_term)
#     if (!is.null(swap)) {
#       swap_candidate_name <- doctor_names[swap$swap_candidate_doc]
# 
#       # Execute the swap
#       temp <- assignment_table_post_processing[doc_idx, which(assignment_table_post_processing[doc_idx, -1] == term)]
#       assignment_table_post_processing[doc_idx, which(assignment_table_post_processing[doc_idx, -1] == term)] <- swap$swap_candidate_term
#       assignment_table_post_processing[swap$swap_candidate_doc, which(assignment_table_post_processing[swap$swap_candidate_doc, -1] == swap$swap_candidate_term)] <- temp
# 
#       # Append the swap details to the dataframe
#       swap_df <- rbind(swap_df, data.frame(DoctorA = doc_name,
#                                            TermA = term,
#                                            DoctorB = swap_candidate_name,
#                                            TermB = swap$swap_candidate_term,
#                                            PreferenceDifferential = swap$combined_pref_diff)) # Use combined differential
# 
#       # Print the swapped terms and the preference differential
#       cat(sprintf("Swapped term %s of doctor %s with term %s of doctor %s | Combined Preference Differential: %d\n",
#                   term, doc_name, swap$swap_candidate_term, swap_candidate_name, swap$combined_pref_diff)) # Use combined differential
#     }
#   }
# }
# 
# # Calculate the average preference differential if there were any swaps
# if (nrow(swap_df) > 0) {
#   average_preference_differential <- mean(swap_df$PreferenceDifferential)
#   cat(sprintf("Average Combined Preference Differential for all swaps: %f\n", average_preference_differential))
# } else {
#   cat("No swaps were made, so there is no average preference differential.\n")
# }
# 
# # Output the swap dataframe if you need to access it
# gt(swap_df)
...
# ## Calculate Specialty violations
# 
# # Assuming you already have computed for assignment_table
# original_specialty_violations <- sum(!specialty_compliance)
# 
# # Compute specialty compliance for assignment_table_post_processing
# specialty_compliance_post_processing <- apply(assignment_table_post_processing[,-1], 1, function(assignments) {
#   # Map the assignments to their corresponding specialty_status_per_term
#   specialty_terms <- specialty_status_per_term[assignments]
# 
#   # Check if any specialty is assigned more than 2 times (assuming the same rule applies for specialties)
#   max_count <- max(table(specialty_terms), na.rm = TRUE)
#   return(max_count <= 2)
# })
# 
# post_processing_specialty_violations <- sum(!specialty_compliance_post_processing)
# 
# # Print results
# cat(sprintf("Number of specialty violations in original assignment_table: %d\n", original_specialty_violations))
# cat(sprintf("Number of specialty violations in assignment_table_post_processing: %d\n", post_processing_specialty_violations))
...
```{r}
# ## Post processing Assignment table, Preference report and HETI compliatnce post processing ##
# 
# # ADJUSTED Doctors Preference Report
# 
# # Initialize an empty data frame to store the adjusted report
# adjusted_doctor_preference_report <- data.frame(
#   Doctor = character(),
#   Term = character(),
#   Preference_Score = numeric(),
#   stringsAsFactors = FALSE
# )
# 
# # Iterate through each doctor in the post-processing assignment table
# for (i in 1:nrow(assignment_table_post_processing)) {
#   doctor_name <- assignment_table_post_processing$Doctor[i]
# 
#   # Access the doctor's preference row from the preference matrix
#   # Note that we should now find the row index by matching the doctor_name in case the order differs
#   doctor_index <- match(doctor_name, row_names)
#   doctor_preference <- preference_matrix[doctor_index, ]
# 
#   for (adjusted_assignment_num in 2:6) { # Start at 2 to account for the Doctor column being first
#     adjusted_assigned_term <- assignment_table_post_processing[i, adjusted_assignment_num]
#     
#     # Check if adjusted_assigned_term is NA or empty, continue if so
#     if (is.na(adjusted_assigned_term) || adjusted_assigned_term == "") {
#       next
#     }
#     
#     # Find the index of the adjusted_assigned_term in column_names
#     term_index <- match(adjusted_assigned_term, column_names)
#     
#     # Check if term_index is NA, indicating the term was not found
#     if (is.na(term_index)) {
#       warning(sprintf("Adjusted term '%s' for doctor '%s' not found in column names.", adjusted_assigned_term, doctor_name))
#       preference_score <- NA # Assign NA or some default score if needed
#     } else {
#       # Calculate preference score for the adjusted term
#       preference_score <- doctor_preference[term_index]
#     }
#     
#     # If preference_score is NA, skip this term
#     if (is.na(preference_score)) {
#       next
#     }
#     
#     # Build a data frame with the results for this doctor and term
#     adjusted_doctor_report <- data.frame(
#       Doctor = doctor_name,
#       Term = adjusted_assigned_term,
#       Preference_Score = preference_score,
#       stringsAsFactors = FALSE
#     )
#     
#     # Append the doctor's report to the adjusted report data frame
#     adjusted_doctor_preference_report <- rbind(adjusted_doctor_preference_report, adjusted_doctor_report)
#   }
# }
...
```
```{r}
## Adjusted HETI compliance table ##

# # 2. Adjusted HETI Compliance Report
# 
# # Calculate average preference for each doctor across all adjusted assignments
# adjusted_avg_preference <- adjusted_doctor_preference_report %>%
#   group_by(Doctor) %>%
#   summarize(Avg_Score = mean(Preference_Score, na.rm = TRUE))
# 
# # Check if a doctor has been assigned to more than 1 term in any sub-specialty in the adjusted data
# adjusted_sub_specialty_compliance <- apply(assignment_table_post_processing[,-1], 1, function(assignments) {
#   # Map the assignments to their corresponding sub_specialty_status_per_term
#   sub_specialty_terms <- sub_specialty_status_per_term[assignments]
#   
#   # Check if any sub-specialty is assigned more than once
#   max_count <- max(table(sub_specialty_terms), na.rm = TRUE)
#   return(max_count <= 1)
# })
# 
# # Check if a doctor has been assigned to more than 2 terms in any specialty in the adjusted data
# adjusted_specialty_compliance <- apply(assignment_table_post_processing[,-1], 1, function(assignments) {
#   # Map the assignments to their corresponding specialty_status_per_term
#   specialty_terms <- specialty_status_per_term[assignments]
#   
#   # Check if any specialty is assigned more than twice
#   max_count <- max(table(specialty_terms), na.rm = TRUE)
#   return(max_count < 3)
# })
# 
# # Check if a doctor has been assigned to at least 3 "Team Based" terms in the adjusted data
# adjusted_team_based_compliance <- sapply(1:num_doctors, function(i) {
#   doctor_name <- row_names[i]
#   terms_assigned <- adjusted_doctor_preference_report$Term[adjusted_doctor_preference_report$Doctor == doctor_name]
#   
#   team_based_count <- sum(clinical_structure_term[match(terms_assigned, names(clinical_structure_term))] == "Team Based")
#   
#   return(team_based_count >= 3)
# })
# 
# # Check for compliance with the desired term classifications in the adjusted data
# adjusted_term_classifications_compliance <- sapply(1:num_doctors, function(i) {
#   doctor_name <- row_names[i]
#   terms_assigned <- adjusted_doctor_preference_report$Term[adjusted_doctor_preference_report$Doctor == doctor_name]
#   return(check_term_classifications(terms_assigned))
# })
# 
# # Compile the adjusted compliance data into a data frame
# adjusted_compliance_data <- data.frame(
#   Doctor = adjusted_avg_preference$Doctor,
#   Avg_Preference = adjusted_avg_preference$Avg_Score,
#   SubSpecialty_Compliance = adjusted_sub_specialty_compliance,
#   Specialty_Compliance = adjusted_specialty_compliance,
#   Team_Based_Compliance = adjusted_team_based_compliance,
#   Term_Classifications_Compliance = adjusted_term_classifications_compliance
# )
# 
# # Format with gt for the adjusted data
# adjusted_HETI_compliance_table <- adjusted_compliance_data %>%
#   gt() %>%
#   cols_label(
#     Avg_Preference = "Avg. Preference (1-5)",
#     SubSpecialty_Compliance = "Max 1 Term in Sub-Specialty",
...
#   )
# 
# print(adjusted_HETI_compliance_table)


```

```{r}
## ADJUSTED ALLOCATIONS TABLE MODEL SUCCESS ##
...
export_tables(
  ...
  filename = "PGY2_exported_tables.xlsx"
)
