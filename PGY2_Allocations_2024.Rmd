...
raw_preference_data <- read_sheet("https://docs.google.com/spreadsheets/d/1mRHV5Sf6I8suZoXBC3nBv3GEHLdHnve1Vt3KLG_fSbU/edit?usp=sharing")
...
term_master_df <- read_csv("PGY2 2024 Dataframe.csv")
...
# Transform the assignment_table into a long format
assignment_long <- assignment_table %>%
  pivot_longer(cols = -Doctor, names_to = "Assignment", values_to = "Term") %>%
  filter(!is.na(Term)) # Remove NA values

# Calculate specialty compliance
specialty_compliance <- assignment_long %>%
  mutate(Specialty = specialty_status_per_term[Term]) %>%
  group_by(Doctor) %>%
  count(Specialty) %>%
  summarize(Max_Specialty_Count = max(n, na.rm = TRUE)) %>%
  mutate(Specialty_Compliance = Max_Specialty_Count <= 2) %>%
  select(Doctor, Specialty_Compliance)

# Calculate subspecialty compliance
sub_specialty_compliance <- assignment_long %>%
  mutate(Subspecialty = as.character(sub_specialty_status_per_term[Term])) %>%
  group_by(Doctor) %>%
  # Count occurrences of each subspecialty, keeping NA
  count(Subspecialty, sort = TRUE, name = "Subspecialty_Count") %>%
  # Replace NA in Subspecialty with a placeholder (e.g., "None") to differentiate from valid subspecialties
  mutate(Subspecialty = ifelse(is.na(Subspecialty), "None", Subspecialty)) %>%
  group_by(Doctor, Subspecialty) %>%
  summarize(Max_Subspecialty_Count = max(Subspecialty_Count)) %>%
  ungroup() %>%
  # Group by Doctor and determine if any subspecialty (excluding "None") exceeds 1
  group_by(Doctor) %>%
  summarize(Subspecialty_Compliance = all(Max_Subspecialty_Count[Subspecialty != "None"] <= 1)) %>%
  ungroup()

# Calculate team-based compliance
team_based_compliance <- assignment_long %>%
  mutate(Team_Based = clinical_structure_term[Term] == "Team Based") %>%
  group_by(Doctor) %>%
  summarize(Team_Based_Count = sum(Team_Based, na.rm = TRUE)) %>%
  mutate(Team_Based_Compliance = Team_Based_Count >= 3) %>%
  select(Doctor, Team_Based_Compliance)

# Calculate term classifications compliance
# Calculate term classifications compliance
term_classifications_compliance <- assignment_long %>%
  mutate(
    Classification1 = term_classification1[Term],
    Classification2 = term_classification2[Term]
  ) %>%
  group_by(Doctor) %>%
  summarize(
    Combined_Classifications = list(unique(na.omit(c(Classification1, Classification2))))
  ) %>%
  mutate(
    Term_Classifications_Compliance = map_lgl(
  ...
  select(Doctor, Term_Classifications_Compliance)

# Merge compliance data with the average preference data
compliance_data <- avg_preference %>%
  left_join(specialty_compliance, by = "Doctor") %>%
  left_join(sub_specialty_compliance, by = "Doctor") %>%
  left_join(team_based_compliance, by = "Doctor") %>%
  left_join(term_classifications_compliance, by = "Doctor")

# Format with gt
HETI_compliance_table <- compliance_data %>%
  gt() %>%
  cols_label(
    Avg_Score = "Avg. Preference (1-5)",
...
export_tables(
  ...
  filename = "PGY2_Allocations_export.xlsx"
)
