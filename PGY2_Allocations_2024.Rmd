...
check_term_classifications <- function(terms_assigned) {
  ...
  # Combine both classifications into one vector for each term
  combined_classifications <- mapply(FUN = function(x, y) {
    unique(c(x, y))
  }, classification1, classification2)

  # Flatten the list to a vector
  combined_classifications <- unlist(combined_classifications)
  ...
  # Check that each required term is present at least once in the combined classifications
  has_required_terms <- sapply(required_terms, function(term) {
    term %in% combined_classifications
  })
