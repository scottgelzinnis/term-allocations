...
find_suitable_swap <- function(term_to_swap, doctor_id, preference_matrix, specialty_status_per_term) {
  for (swap_candidate_doc in 1:num_doctors) {
    # Skip if checking the same doctor
    if (swap_candidate_doc == doctor_id) next
    
    for (swap_candidate_term in assignment_table_post_processing[swap_candidate_doc, -1]) {
      
      # Handle NA values and skip if the term is from the same specialty
      term1_specialty <- specialty_status_per_term[term_to_swap]
      term2_specialty <- specialty_status_per_term[swap_candidate_term]
      
      # Skip if either of the terms doesn't have a specialty
      if (is.na(term1_specialty) || is.na(term2_specialty)) next
      
      # Skip if the term is from the same specialty
      if (term1_specialty == term2_specialty) next
      
      # Check preference difference criteria
      ...
      if (abs(original_preference_doc - swap_preference_doc) <= 10 && 
          abs(original_preference_swap_candidate - swap_preference_swap_candidate) <= 10) {
        return(list(swap_candidate_doc, swap_candidate_term))
      }
    }
  }
  
  # If no suitable swap found
  return(NULL)
}

# Use the function in the post-processing step
violating_docs = which(!specialty_compliance)
for (doc in violating_docs) {
  doc_assignments <- unlist(assignment_table_post_processing[doc, -1]) # Ensure it's a vector, not a list
  for (term in doc_assignments) {
    ...
    if (is.na(term_specialty)) next # Skip if the term doesn't have a specialty
    
    if (specialty_counts[term_specialty] > 2) {
      swap = find_suitable_swap(term, doc, preference_matrix, specialty_status_per_term)
      if (!is.null(swap)) {
        # Execute the swap
        temp = assignment_table_post_processing[doc, which(assignment_table_post_processing[doc, -1] == term)]
        assignment_table_post_processing[doc, which(assignment_table_post_processing[doc, -1] == term)] = swap[[2]]
        assignment_table_post_processing[swap[[1]], which(assignment_table_post_processing[swap[[1]], -1] == swap[[2]])] = temp

        # Print the swapped terms
        cat(sprintf("Swapped term %s of doctor %d with term %s of doctor %d\n", term, doc, swap[[2]], swap[[1]]))
      }
    }
  }
}
...
## SWAP FOR SUBSPECIALTY TERM HETI ADHERANCE ##

find_subspecialty_swap <- function(term_to_swap, doctor_id, preference_matrix, sub_specialty_status_per_term) {
  for (swap_candidate_doc in 1:num_doctors) {
    # Skip if checking the same doctor
    if (swap_candidate_doc == doctor_id) next
    ...
    for (swap_candidate_term in assignment_table_post_processing[swap_candidate_doc, -1]) {
      ...
      # Handle NA values and skip if the term is from the same sub-specialty
      term1_sub_specialty <- sub_specialty_status_per_term[term_to_swap]
      term2_sub_specialty <- sub_specialty_status_per_term[swap_candidate_term]
      
      # Skip if either of the terms doesn't have a sub-specialty
      if (is.na(term1_sub_specialty) || is.na(term2_sub_specialty)) next
      
      # Skip if the term is from the same sub-specialty
      if (term1_sub_specialty == term2_sub_specialty) next
...
# Use the function in the post-processing step
violating_docs_subspecialty = which(!sub_specialty_compliance)
for (doc in violating_docs_subspecialty) {
  doc_assignments <- unlist(assignment_table_post_processing[doc, -1]) # Ensure it's a vector, not a list
  for (term in doc_assignments) {
    sub_specialty_counts <- table(sub_specialty_status_per_term[doc_assignments])
    term_sub_specialty <- sub_specialty_status_per_term[term]
    
    if (is.na(term_sub_specialty)) next # Skip if the term doesn't have a sub-specialty
    
    if (sub_specialty_counts[term_sub_specialty] > 1) {
      swap = find_subspecialty_swap(term, doc, preference_matrix, sub_specialty_status_per_term)
      if (!is.null(swap)) {
        # Execute the swap
        temp = assignment_table_post_processing[doc, which(assignment_table_post_processing[doc, -1] == term)]
        assignment_table_post_processing[doc, which(assignment_table_post_processing[doc, -1] == term)] = swap[[2]]
...
```
```{r}
## Calculate subspecialty violations

# Already computed for assignment_table
original_violations <- sum(!sub_specialty_compliance)

# Compute for assignment_table_post_processing
sub_specialty_compliance_post_processing <- apply(assignment_table_post_processing[,-1], 1, function(assignments) {
  # Map the assignments to their corresponding sub_specialty_status_per_term
  sub_specialty_terms <- sub_specialty_status_per_term[assignments]
...
post_processing_violations <- sum(!sub_specialty_compliance_post_processing)

# Print results
cat(sprintf("Number of violations in original assignment_table: %d\n", original_violations))
cat(sprintf("Number of violations in assignment_table_post_processing: %d\n", post_processing_violations))
...
assignment_table_updated <- left_join(survey_data, assignment_table, by = "Doctor")
  
  # Now, continue with the rest of your code to add to the tables_list
    tables_list <- list(
    AssignmentTable = assignment_table_updated,
