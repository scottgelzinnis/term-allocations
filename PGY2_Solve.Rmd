...
# Calculate the range of assigned preference values for each doctor
range_per_doctor <- apply(assigned_preference_values, 1, function(row) max(row) - min(row))

# Calculate the standard deviation of assigned preference values for each doctor
sd_per_doctor <- apply(assigned_preference_values, 1, sd)

# Compute the average range and SD across all doctors
average_range <- mean(range_per_doctor)
...
evaluation_data <- data.frame(
  Metric = c("At least 1 term in Top 10 (%)", 
             ...
             "Average Range of Assigned Preferences",
...
evaluation_data$Value <- round(evaluation_data$Value, 1)
